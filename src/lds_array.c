...
LDS_RET new_lds_array(struct lds_array *a_ret, size_t init_size,
                      size_t elem_size,
...
static LDS_RET resize_lds_array(lds_array *lds_arr, size_t size,
                                const struct lds_allocator *alloc_nullable) {
  void *resized_buf = lds_realloc(lds_arr->arr, size, alloc_nullable);
...
static inline void write_lds_array(lds_array *lds_arr, size_t pos,
                                   const void *element) {
  memcpy((char *)lds_arr->arr + lds_arr->elem_size * pos, element,
         lds_arr->elem_size);
}

LDS_RET add_lds_array(lds_array *lds_arr, const void *element,
                      const struct lds_allocator *alloc_nullable) {
  if (element == NULL) {
  ...
  if (lds_arr->curr_size + 1 >
      lds_arr->max_size) { // Array exceeds maximum size.
    LDS_RET res =
...
LDS_RET set_lds_array(lds_array *lds_arr, size_t pos, const void *element) {
