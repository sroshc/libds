...
inline void* lds_malloc(size_t size, struct lds_allocator* alloc_nullable){
    return alloc_nullable == NULL ? GLOBAL_LDS_ALLOC.malloc(size): alloc_nullable.malloc(size);
}

inline void* lds_realloc(void* ptr, size_t size, struct lds_allocator* alloc_nullable){
    return alloc_nullable == NULL ? GLOBAL_LDS_ALLOC.realloc(ptr, size): alloc_nullable.realloc(ptr, size);
}
